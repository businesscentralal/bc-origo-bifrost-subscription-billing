...
codeunit 10035046 "Sub Bil CrDocs Impl ori" implements "Msg Interface ori"
{
    var
        ...
        GroupByCustomerOnlyErr: Label '''groupBy'' = ''Customer'' only applies when the pending proposal lines belong to customer contracts.', Comment = 'is-IS=''groupBy'' = ''Customer'' á aðeins við þegar óafgreiddar tillögulínur tilheyra viðskiptavinasamningum.';
        NothingPendingMsg: Label 'There are no unbilled proposal lines for Billing Template ''%1''. Run Subscription.Billing.CreateProposal first.', Comment = '%1 = billing template code||is-IS=Það eru engar óreikningsfærðar tillögulínur fyrir reikningssniðmátið ''%1''. Keyrðu Subscription.Billing.CreateProposal fyrst.';
        NotRolledBackErr: Label 'The billing run failed after Business Central had already created the documents listed in ''documents''. Business Central commits each document as it is created, so those documents still exist and were not rolled back. Review them before running this template again. Underlying error: %1', Comment = '%1 = the underlying Business Central error||is-IS=Reikningskeyrslan mistókst eftir að Business Central hafði þegar búið til skjölin sem talin eru upp í ''documents''. Business Central staðfestir hvert skjal um leið og það verður til, svo þau skjöl eru enn til og hafa ekki verið afturkölluð. Yfirfarðu þau áður en þetta sniðmát er keyrt aftur. Undirliggjandi villa: %1';
        DescriptionLbl: Label 'Turns the unbilled Billing Line rows under a Billing Template into sales or purchase documents. Reports the documents created and how many proposal lines were processed.', MaxLength = 250, Comment = 'is-IS=Umbreytir óreikningsfærðum reikningslínum undir reikningssniðmáti í sölu- eða innkaupaskjöl. Skilar þeim skjölum sem urðu til og fjölda tillögulína sem unnið var úr.';
